...
common tools are used? Are there any important aspects that makes memory
...
Knowing the size of each object is also important. So the size of each object that you write in your program is,
in some cases, *known at compile-time*.

The `zig` compiler cares more about knowing the size of a particular object
...
automatically knows the size of this object. But if the value of this object is not
...
And because of that, the `zig` compiler does not need to known at compile-time the exact value of
...
Is impossible to know at compile-time the value of this particular argument. And it also is impossible to know the size
...
used to store values whose size might grow during the execution (runtime) of your program [@jenny2022].
...
One important limitation of the stack, is that, only objects whose size is known at compile-time can be
...
on objects whose size might grow during the execution of your program.
...
of memory that you, have complete control over. This makes the heap a
more flexible type of memory, but it also makes it harder to work with it. Because you, 
is responsible for managing everything related to it. Including where the memory is allocated,
...
To store an object in the heap, you, needs to explicitly tells Zig to do so,
...
::: {.callout-important}
Every memory you allocate in the heap needs to be explicitly freed by you.
:::
...
1. every object (constant or not) whose size **is known at compile-time** is stored in the stack space for the current scope.
...
That is why, the first argument of this function is an allocator object that you, gives
...
that provides the allocator for the function to use.
...
But in reality, there are two very common instances where this "fixed size limitation" of the stack is a deal braker:
...
really does is helping you to free all the memory you allocated multiple times with just a single command. In the example
